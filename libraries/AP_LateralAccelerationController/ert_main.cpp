...
#include "lateral_acceleration_controller.h" // Model header file
...
void rt_OneStep(void)
{
  ...
  // Check for overrun
  if (OverrunFlag) {
    rtmSetErrorStatus(lateral_acceleration_contro_Obj.getRTM(), "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  // Initialize model
  lateral_acceleration_contro_Obj.initialize();
  ...
  fflush((nullptr));
  while (rtmGetErrorStatus(lateral_acceleration_contro_Obj.getRTM()) == (nullptr))
  {
    //  Perform application tasks here
  }

  // Terminate model
  lateral_acceleration_contro_Obj.terminate();
  return 0;
}
